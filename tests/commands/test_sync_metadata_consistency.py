...
from bugster.utils.yaml_spec import load_yaml_specs, save_yaml_specs
...
"""
    ...
    mock_specs_service.get_remote_specs.return_value = {}  # No remote specs
    mock_specs_service.upload_specs = MagicMock()

    # Mock the SpecsService constructor
    ...
    monkeypatch.setattr(
        "bugster.commands.sync.SpecsService", mock_specs_service_constructor
    )
    ...
    # Verify upload_specs was called
    assert mock_specs_service.upload_specs.called

    # Get the uploaded spec metadata
    upload_call_args = mock_specs_service.upload_specs.call_args[0]
